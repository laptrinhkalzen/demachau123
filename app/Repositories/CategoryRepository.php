...
class CategoryRepository extends AbstractRepository {
    ...
    public function readPlatformCategory() {
        return $this->model->where('status', '1')->where('parent_id', '255')->get();
    }
    ...
    public function getGenreCategory($id) {
        $category = \DB::table('product_category')->where('product_id',$id)->pluck('category_id');   
        return $this->model->whereIn('id',$category)->where('parent_id',257)->pluck('id')->first();
    }
